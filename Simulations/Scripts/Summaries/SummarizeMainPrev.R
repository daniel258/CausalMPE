#############################################
## CausalMPE project
# This script combines the results from multiple simulation scripts carried out on my computer.
...
all.res <- matrix(nr = length(all.patts)*1000, nc = 23)
keep <- list(c("keep", "all.patts","all.res", "ii"))
#j <- 1
setwd("../RawResultsPrev/Main/")
for (ii in 1:length(all.patts))
{
 # j <- j + 1
  temp.patt <- all.patts[ii] 
  load(paste0("CMPEprev",temp.patt,".RData"))
  ...
  all.res[((ii - 1) * 1000 + 1):(ii * 1000) , 20] <- or.approx1
  all.res[((ii - 1) * 1000 + 1):(ii * 1000) , 21] <- or.approx2
  all.res[((ii - 1) * 1000 + 1):(ii * 1000) , 22] <- or.approx.true1
  all.res[((ii - 1) * 1000 + 1):(ii * 1000) , 23] <- or.approx.true2
...
write.csv(all.res, "all.res.main.prev.csv", row.names = F)
