...
#j <- 1
setwd("/Users/danielnevo/Dropbox/CausalMPE/R/RawResultsNonRare/Main")
for (ii in 1:length(all.patts))
{
 # j <- j + 1
  temp.patt <- all.patts[ii] 
  load(paste0("CMPEnonrare",temp.patt,".RData"))
...
write.csv(all.res, "all.res.main.nonrare.csv", row.names = F)
