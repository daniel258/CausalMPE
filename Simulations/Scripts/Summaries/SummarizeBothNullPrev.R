...
#j <- 1
setwd("/Users/danielnevo/Dropbox/CausalMPE/R/RawResultsPrev/BothNull/")
for (ii in 1:length(all.patts))
{
 # j <- j + 1
  temp.patt <- all.patts[ii]
  load(paste0("CMPEprev",temp.patt,"null.RData"))
...
write.csv(all.res, "all.res.null.prev.csv", row.names = F)
