...
all.patts <- c("A", "B", "C", "D", "E", "F")
all.res <- matrix(nr = length(all.patts)*1000, nc = 19)
keep <- list(c("keep", "all.patts","all.res", "ii"))
#j <- 1
setwd("/Users/danielnevo/Dropbox/CausalMPE/R/RawResultsNonRare/BothNull/")
for (ii in 1:length(all.patts))
{
 # j <- j + 1
  temp.patt <- all.patts[ii]
  load(paste0("CMPEnonrare",temp.patt,"null.RData"))
...
write.csv(all.res, "all.res.null.nonrare.csv", row.names = F)
