...
#j <- 1
setwd("/Users/danielnevo/Dropbox/CausalMPE/R/RawResultsNonRare/Single")
for (ii in 1:length(all.patts))
{
 # j <- j + 1
  CatIndex(ii)
  temp.patt <- all.patts[ii]
  load(paste0("CMPEnonrareSingle",temp.patt,".RData"))
...
write.csv(all.res, "all.res.single.nonrare.csv", row.names = F)
